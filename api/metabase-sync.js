...
//
// A single client's Metabase Account field can also list several Metabase
// sub-accounts, comma-separated (e.g. "acct_men, acct_women") — useful when
// one team is tracked as separate accounts in Metabase but run as one unit
// in the planner. Their values are summed into one Starting Target.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const client of due) {
      // Metabase Account can list several sub-accounts (comma-separated) that
      // should be summed into one number — e.g. a client run as one team but
      // tracked as separate accounts in Metabase (americaneagle_men,
      // americaneagle_women). Each part is looked up independently; whatever
      // is found gets summed, and any part that's missing is called out
      // rather than silently dropped.
      const accountParts = String(client.metabaseAccount).split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
      const foundParts = [], missingParts = [];
      let value = 0;
      for (const part of accountParts) {
        const partValue = metabaseData.get(part);
        if (partValue === undefined) missingParts.push(part);
        else { value += partValue; foundParts.push(part); }
      }
      ...
      alreadySyncedMap[client.id] = today;
      const resultEntry = { client: client.name, status: 'updated', startingTarget: value };
      if (accountParts.length > 1) resultEntry.summedFrom = foundParts;
      if (missingParts.length > 0) resultEntry.missingSubAccounts = missingParts;
